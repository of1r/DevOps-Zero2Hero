...
echo "# Section Documentation" > $DIRECTORY/TOC.md
echo -e "\n" >> $DIRECTORY/TOC.md

# Process each markdown file except README.md
find "$DIRECTORY" -maxdepth 1 -name "*.md" ! -name "README.*" -print0 | sort -z | while IFS= read -r -d '' file; do
    ...
    echo "## [$title]($filename)" >> $DIRECTORY/TOC.md
    ...
    grep -E '^(\s*)-\s*\[.*\]\(#.*\)' "$file" | while read -r line; do
        ...
        echo " - [$link_text]($filename$anchor)" >> $DIRECTORY/TOC.md
    done
